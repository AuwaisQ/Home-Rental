...
class XController extends GetxController {
  ...
  var appMetaList = <ApplicationMeta>[].obs;

  final AllProvider _provider = AllProvider();
  AllProvider get provider => _provider;

  final myPref = Get.find<MyPref>();
  var transactionId = 'TXID_${DateTime.now().second}${DateTime.now().minute}${DateTime.now().hour}${DateTime.now().millisecond}'.obs;
  ...
  getUserById() async {
    try {
      ...
      if (loggedIn.value) {
        ...
        _provider.pushResponse('api/get_user?lt=${MyTheme.pageLimit}', jsonBody)!.then((Response? response) {
          if (response != null && response.statusCode == 200) {
            dynamic dataresult = jsonDecode(response.bodyString!);
            //logPrint(_result);
            if (dataresult['code'] == '200') {
  ...
  updateUserById(final String action, final String about, final String fullname) async {
    try {
      ...
      final response = await _provider.pushResponse('api/update_user_byid', jsonBody);
      if (response != null && response.statusCode == 200) {
        //logPrint(response.bodyString!);
        Future.microtask(() async {
          if (action == 'change_password') {
            await notificationFCMManager.firebaseAuthService.firebaseUpdatePassword(fullname);
          }
          getUserById();
  ...
  addProperty(
    ...
    try {
      ...
      final response = await _provider.pushResponse('api/addrent_post', jsonBody);
      if (response != null && response.statusCode == 200) {
        debugPrint('Property Added');
        dynamic dataResult = jsonDecode(response.bodyString!);
        print("Response - $dataResult");
  ...
  terminateAccount() async {
    try {
      ...
      final response = await _provider.pushResponse('user/user_delete', jsonBody);
      if (response != null && response.statusCode == 200) {
        setIndexBar(0);
        doLogout();
        Future.delayed(const Duration(milliseconds: 1000), () {
          EasyLoading.showSuccess('Account Terminated successfully...');
          EasyLoading.dismiss();
  ...
  final NotificationFCMManager notificationFCMManager = NotificationFCMManager.instance;
