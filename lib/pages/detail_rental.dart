...
class DetailRental extends StatelessWidget {
                      ...
                      Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Text(
                              "${myPref.pCurrency.val} ${e.price!} /Month",
                              style: const TextStyle(
                                  fontSize: 14,
  ...
  static showDialogBooking(final UserModel user, final RentalModel rental) {
    ...
    final XController controller = XController.to;
    //UPI
    Future<void> initiatePayment(ApplicationMeta appMeta) async {
      // Set up UPI details
      final UpiTransactionResponse response = await UpiPay.initiateTransaction(
        amount: rental.unitPrice.toString(),
        app: appMeta.upiApplication,
        receiverName: 'Second Home',
        receiverUpiAddress: 'Q79491606@ybl',
        // receiverUpiAddress: '7224985288@okbizaxis',//Bharat Kirana Merchant UPI
        transactionRef: controller.transactionId.value,
        transactionNote: 'Second Home Rental Apartment Booking',
      );
      // Handle the UPI transaction response
      if (response.status == UpiTransactionStatus.success) {
        print('Payment Response-$response');
        doPostNewRent(controller, rental, _date.text.trim().toString(), 'UPI', "Method Online");
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      InkWell(
                        onTap: () {
                          final fullnm = _fullname.text.trim();
                          if (fullnm.isEmpty || fullnm.length < 3) {
                            EasyLoading.showToast('Fullname invalid...');
                            return;
                          }else if(controller.appMetaList.isEmpty){
                            Get.snackbar(
                              'Payment Error',
                              'Please Install A UPI App',
                                      ...
                                      Obx(() {
                                        return Expanded(
                                          child: GridView.builder(
                                              ...
                                              itemCount: controller.appMetaList.length,
                                              itemBuilder: (BuildContext context, int index) {
                                                return InkWell(
                                                  onTap: ()async{
                                                    print('UPI Package Name - ${controller.appMetaList[index].upiApplication}');
                                                    try {
                                                      ///checks if the app is installed on your mobile device
                                                      bool isInstalled = await AppLauncher.hasApp(androidApplicationId: controller.appMetaList[index].packageName,);
                                                      print('isInstalled =$isInstalled');
                                                      if (isInstalled) {
                                                        //UPI Trigger Function
                                                        initiatePayment(controller.appMetaList[index]);
                                                        // AppLauncher.openApp(androidApplicationId: controller.appMetaList[index].packageName,);
                                                      } else {
                                                        ///if the app is not installed it lunches google play store so you can install it from there
                                                        Get.snackbar('Bharat Kirana', 'This App is not installed');
                                                      }
                                                    } catch (e) {
                                                      print(e);
                                                    }
                                                  ...
                                                  child: Column(
                                                    children: [
                                                      controller.appMetaList[index].iconImage(48),
                                                      Text(controller.appMetaList[index].upiApplication.appName),
                                                    ],
                                                  ),
                                                );
                                              }
                      ...
                      //           if (stepProcess.value == 2 && idxMethod.value == 0) {
                      //             stepProcess.value = 1;
                      //             dataFormCard.value = {
                      //               "no": "",
                      //               "exp": "",
                      ...
                      //             //     dateRange.length < 3) {
                      //             //   EasyLoading.showToast('Date Range selection invalid...');
                      //             //   return;
                      //             // }
                      //
                      ...
                      //               await doPostNewRent(XController.to, rental, dateRange, "Cash", "Method Cash");
                      //               //Get.back();
                      //             } else if ((stepProcess.value == 2 || stepProcess.value == 3) &&
                      //                 (idxMethod.value == 0 || idxMethod.value == 1)) {
                      //               String em = _email.text.trim();
                      //               if (idxMethod.value == 1 && (em.isEmpty || !GetUtils.isEmail(em))) {
                      ...
                      //                   rental,
                      //                   dateRange,
                      //                   idxMethod.value == 1 ? "Paypal" : "Credit Card",
                      //                   idxMethod.value == 1 ? em : jsonEncode(dataFormCard));
                      ...
                      //                 Obx(
                      //                   () => Text(
                      //                     (stepProcess.value == 2 || stepProcess.value == 3) && (idxMethod.value == 0 || idxMethod.value == 1)
                      //                         ? "Payment"
                      //                         : "Book",
  ...
  static showDialogSuccess() {
    final XController xx = XController();
    return showCupertinoModalBottomSheet(
      context: Get.context!,
      isDismissible: false,
      builder: (context) => Container(
                        ...
                        child: Container(
                          ...
                          child: Column(
                            children: [
                              ...
                              spaceHeight20,
                              Image.asset("assets/green-success.gif", width: 180),
                              spaceHeight20,
                            ],
                          ),
                        ),
  ...
  static Widget childFormCreditCard(final RentalModel rental, final MyPref myPref) {
    return Column(
      children: [
        Obx(
          () => CreditCardWidget(
  ...
  static Widget childFormBooking(final RentalModel rental, final MyPref myPref) {
    return Column(
      children: [
        //FullName
        Container(
          ...
          child: Container(
            ...
            child: TextField(
                ...
                decoration:
                    MyTheme.inputForm('Full Name', Get.theme.canvasColor)),
          ),
        ),
        spaceHeight10,
        // InkWell(
            ...
            children: [
              SizedBox(
                ...
                child: Row(
                  children: [
                    ...
                    spaceWidth5,
                    Text("${rental.address}", style: TextStyle(fontSize: 11, color: Colors.grey[500]),
                    ),
                  ],
                ),
              ),
              Expanded(
                ...
                child: Padding(
                  ...
                  child: Text("${myPref.pCurrency.val}. ${MyTheme.formatCounterNumber(rental.price!)} /Month",
        ...
        Container(
          ...
          child: Row(
            ...
            children: [
              Row(
                children: [
                  const Icon(Feather.book, size: 12, color: Colors.grey),
                  spaceWidth5,
                  Text("Booking Amount",
          ...
          child: SfDateRangePicker(
            ...
            onSelectionChanged: (DateRangePickerSelectionChangedArgs args) {
              ...
              if (args.value is PickerDateRange) {
                // String dtarange = '${DateFormat('dd/MM/yyyy').format(args.value.startDate)} - ${DateFormat('dd/MM/yyyy').format(args.value.endDate ?? args.value.startDate)}';
                String dtarange = '${DateFormat('dd/MM/yyyy').format(DateTime.now())} - ${DateFormat('dd/MM/yyyy').format(DateTime.now())}';
                //debugPrint(_range);
